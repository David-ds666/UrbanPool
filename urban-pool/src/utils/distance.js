/**
 * Client-side distance calculator — uses Photon geocoding (India-biased) → OSRM → Haversine → heuristic
 */

// India bounding box — any result outside this is wrong and discarded
...
// ── Photon geocoding biased to India (lat/lon of India's center) ──
async function geocode(address) {
  try {
    // bias=India centre, limit=5 so we can scan for a valid Indian result
    const url = `https://photon.komoot.io/api/?q=${encodeURIComponent(address)}&limit=5&lang=en&lat=22.0&lon=78.0`;
    const response = await fetch(url);
    const data = await response.json();
    if (!data?.features?.length) return null;

    // Pick the FIRST result that falls inside India's bounding box
    for (const feature of data.features) {
      const [lng, lat] = feature.geometry.coordinates;
      if (withinIndia(lat, lng)) return { lat, lng };
    }
    return null; // no Indian result — will fall back to heuristic
  } catch {
    return null;
  }
}
...
// ── Haversine formula (straight-line × 1.3 road factor) ──
function haversine(lat1, lng1, lat2, lng2) {
  ...
  return parseFloat((R * c * 1.3).toFixed(1)); // ×1.3 road winding factor
}

// ── Heuristic fallback: city-name matching with known Indian coordinates ──
const COORDS_DB = {
  ...
  'bihar':            { lat: 25.0961, lng: 85.3131 }, 'west bengal':  { lat: 22.9868, lng: 87.8550 },
  ...
  'assam':            { lat: 26.2006, lng: 92.9376 }, 'sikkim':       { lat: 27.5330, lng: 88.5122 },
  'chandigarh':       { lat: 30.7333, lng: 76.7794 }, 'mohali':       { lat: 30.7046, lng: 76.7179 },
  'aerocity':         { lat: 30.6784, lng: 76.7928 }, 'gmada':        { lat: 30.6784, lng: 76.7928 },
  'airport':          { lat: 30.6702, lng: 76.7886 }, 'chachu majra': { lat: 30.6500, lng: 76.8100 },
  'sahibzada':        { lat: 30.7046, lng: 76.7179 }, 'ajit singh':   { lat: 30.7046, lng: 76.7179 },
  'kharar':           { lat: 30.7460, lng: 76.6499 }, 'zirakpur':     { lat: 30.6428, lng: 76.8174 },
  ...
  'amritsar':         { lat: 31.6340, lng: 74.8723 }, 'bathinda':     { lat: 30.2110, lng: 74.9455 },
  'delhi':            { lat: 28.7041, lng: 77.1025 }, 'new delhi':    { lat: 28.6139, lng: 77.2090 },
  ...
  'jaipur':           { lat: 26.9124, lng: 75.7873 }, 'noida':        { lat: 28.5355, lng: 77.3910 },
  'gurgaon':          { lat: 28.4595, lng: 77.0266 }, 'gurugram':     { lat: 28.4595, lng: 77.0266 },
  'shimla':           { lat: 31.1048, lng: 77.1734 }, 'manali':       { lat: 32.2396, lng: 77.1887 },
...
function heuristicDistance(pickupStr, dropStr) {
  ...
  const keys = Object.keys(COORDS_DB).sort((a, b) => b.length - a.length); // longest first
  ...
  if (pC && dC) {
    ...
    return dist < 0.5 ? 8.0 : dist; // same-location fallback
  }
 ...
 * Chain: India-biased Photon geocoding → OSRM road distance → Haversine → heuristic
 */
export async function calculateDistanceAsync(pickupStr, dropStr) {
  // Step 1: geocode both addresses (India-biased, validated within India bounds)
  ...
  if (pickupCoords && dropCoords) {
    // Step 2: OSRM real road distance
    ...
    // Step 3: Haversine fallback (geocoded coords are within India, so this is accurate)
    const hav = haversine(pickupCoords.lat, pickupCoords.lng, dropCoords.lat, dropCoords.lng);
    // Sanity check: if haversine result is suspiciously large (>500 km for same-state trips),
    // the geocoding picked a wrong city — use heuristic instead
    if (hav > 500) return heuristicDistance(pickupStr, dropStr);
    return hav;
  }

  // Step 4: geocoding failed or returned non-Indian result → local heuristic
  return heuristicDistance(pickupStr, dropStr);
}

/**
 * Synchronous heuristic distance (for immediate fallback/non-async contexts).
 */
